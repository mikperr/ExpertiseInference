...
def generate_expertise():
    ...
    import matplotlib.pyplot as plt
    def sigmoid(x,a,b):
      return 0.98*(1 / (1 + np.exp(-a*(x-b))))+0.01
    a=10
    ...
    zeta=np.random.rand(len(vocab))*(inter_b-inter_a)+inter_a
    ...
    zeta_generate=np.zeros(len(dct))
    for i in range(len(dct)):
        zeta_generate[i]=zeta[0,wordid[i]]
    ...
    return X,p_generate,theta_generate,phi_generate,zeta,gamma_generate
